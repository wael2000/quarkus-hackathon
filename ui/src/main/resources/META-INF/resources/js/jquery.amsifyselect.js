...
(function(factory){
	...
	AmsifySelect.prototype = {
        ...
        createHTML : function() {
			...
			*/
			if(this.isSearchable) {
				...
				var search                = '<input type="text" class="form-control '+this.classes.search.substring(1)+'" placeholder="Search here..."/>';
			...
			var clear                 = '<button class="btn btn-primary '+this.classes.clear.substring(1)+' '+this.clearClass+'">Clear</button>';
			this.selectors.clear      = $(clear).appendTo(this.selectors.operations);

			var close                 = '<button class="btn btn-secondary '+this.classes.close.substring(1)+' '+this.closeClass+'">Close</button>';
		...
		getInputType : function(value) {
			...
			return '<input type="'+type+'" name="'+this.name+'" class="form-control '+inputClass+'" value="'+value+'"/>';
		},
